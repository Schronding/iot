...
(Replace PORT with the name of the serial port to use. For this workspace, the preferred FTDI port is `/dev/serial/by-id/usb-FTDI_FT232R_USB_UART_A5069RR4-if00-port0` so VS Code and ESP-IDF stay on the stable device path instead of `ttyS0` or a changing `ttyUSB*` name.)
